...
- question や解説の中で特定の選択肢に触れるときは、その選択肢の文言を「」で引用して参照する。ラベル・記号・番号・並び順による参照
（「選択肢A」「選択肢1」「④」「上から2番目」「最後の選択肢」など）は禁止。
長い選択肢は先頭の識別できる部分までの引用でよい（要約や言い換えは不可）。「正解の選択肢」「誤りの選択肢」のような役割による言及は問題ない
  例（可）: 「any からの変換」を選んだ場合は〜
  例（不可）: 選択肢Aは〜 / 2番目の選択肢は〜
- slug は英語のケバブケースで出力
${tagRule}
- 解説は600〜1000文字程度のHTMLリッチテキスト。最初の段落で正解と要点を簡潔に述べ、その後で詳細に入る
- 解説は <h3> の小見出しで構造化し、<p>, <strong>, <code>, <pre><code>, <ul><li> を活用して読みやすくする。各誤答がなぜ誤りか必要に応じていいれる。
- 解説に「よくある誤解」または「実務での注意点」を1つ含める、ただし小見出しやこの項目についてはそのまま「よくある誤解」または「実務での注意点」とはしない
（よくある抽象的な見出しは読み手に不快なので避けたい）、に具体的なその内容に関するものを入れ意味を持たせる
- コード例を含める場合は最小限で動くものにし、重要な行にはコメントで挙動を補足する
- 解説の末尾に「参考」として、公式ドキュメント・一次情報へのリンクを <p><a href="URL">タイトル</a></p> の形式で1〜2件含める。日本語版のページがあれば日本語版を優先する。URLが正確だと確信できる場合のみ記載し、少しでも不確かなら省略してよい（存在しないURLの創作は厳禁）`;
}
