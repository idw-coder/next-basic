...
> = {
  5: {
    ...
    label: 'text-emerald-500/70 dark:text-emerald-400/60',
  },
  10: {
  ...
  15: {
    ...
    label: 'text-violet-500/70 dark:text-violet-400/60',
  },
  20: {
    bg: 'bg-amber-50 dark:bg-amber-500/10',
    border: 'border-amber-200 dark:border-amber-700',
    text: 'text-amber-600 dark:text-amber-400',
    label: 'text-amber-500/70 dark:text-amber-400/60',
  },
};
...
export default function RandomQuizClient({
  ...
  const [completedSession, setCompletedSession] = useState<RandomQuizSession | null>(null);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const picked = shuffled.slice(0, Math.min(quizCount, shuffled.length));
          ...
          <div className="grid grid-cols-4 gap-2 sm:gap-3">
            {QUIZ_COUNTS.map((count) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn('text-xl sm:text-2xl font-extrabold tabular-nums', style.text)}
                  >
                    {count}
                  </div>
                  <div className={cn('text-[10px] sm:text-xs font-medium', style.label)}>問</div>
                </button>
              );
            })}
          </div>
    ...
    <div className="space-y-8">
      <Card className="text-center">
        ...
        <CardContent className="space-y-6">
          <div className="text-center">
            <div className="text-5xl font-extrabold text-primary">{scorePercent}%</div>
            <div className="text-sm text-muted-foreground mt-1">正答率</div>
          </div>
          ...
          <div className="flex justify-center gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold">{totalCount - correctCount}</span>
              <span className="text-sm text-muted-foreground">不正解</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-3">
              <Button onClick={onChangeSettings} variant="outline" size="lg" className="flex-1">
      ...
      <div className="space-y-3">
        ...
        {session.quizzes.map((quiz, index) => {
          ...
          return (
            <Link key={quiz.id} href={`/quiz/${quiz.categorySlug}/${quiz.id}`}>
