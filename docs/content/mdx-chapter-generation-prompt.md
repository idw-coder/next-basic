...
> 参照用メモ（2026-08-08）: このファイルは旧式の単体プロンプト。現在の一次情報は `docs/books.md` の「AIによるコンテンツ作成ガイド」と「MDX記述ルール」。新規章の作成では `docs/books.md` を優先してください。
...
- [ ] 表のセルに長い説明を詰め込まず、短い要約だけを置いて詳しい説明は本文やCalloutに逃がしている
- [ ] Calloutの閉じタグ前に空行がある
