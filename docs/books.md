...
- 追加した章で新しい主要概念を扱ったなら、**本内検索サジェストを見直す**（次節）

### 章を追加したら検索サジェストを見直す

`src/app/books/_constants/searchSuggestions.ts` の `BOOK_SEARCH_SUGGESTIONS` は、**新しい本を追加したときだけ触るものではない。** 既存の本に章を追加した場合も、その章で新しく扱った主要概念がサジェストに無ければ追加してよい。むしろ追加した章は既存読者にとって最も見つけにくいため、サジェストは有効な導線になる。

判断基準はシンプルに、次の両方を満たす語だけを足す。

- **読者がその語で検索しに来ると想像できる** — API名・キーワード・エラー名・概念名など（例: `implements`、`safeParse`、`useCallback`）
- **その語を主題として扱う章が実在する** — 本文で一言触れただけの語は足さない

```ts
// 例: TypeScript本に「クラスとimplements」章を追加したとき
typescript: ['ユニオン型', 'ジェネリクス', '型ガード', 'Utility Types', 'tsconfig', 'implements'],
```

件数の目安は1冊あたり10〜15件。サイドバーは配列を全件描画する（`BookSidebar.tsx` に上限のスライスは無い）ため、増やしすぎると検索欄の下が長くなる。上限に達している本で新しい語を足したいときは、**古い語を1つ落として入れ替える**。

追加後は、対象の `/books/{bookSlug}` か章ページを開き、検索欄をフォーカスして候補に出ることを確認する。サジェストは検索そのものではないので、語を足しても本文がヒットしなければ意味がない。**その語で実際に本文がヒットすることまで確認する。**
...
MDN（developer.mozilla.org/ja）や各技術の公式ドキュメントへのリンクは、読者が一次情報へ進む導線として適宜入れる。ただし以下を守る。

- **本文の説明を完結させてから置く。** 「詳しくはMDNへ」で説明を省略しない。この教科書はMDN代替を目指すため（「メソッド・APIを解説するときは…」の節を参照）、リンクは深掘り用であって説明の代わりではない
- **置く場所は節末か章末。** 説明の途中に挟んで読者を離脱させない。章末にまとめる場合は「## 参考リンク」として2〜4本程度に絞る
- **リンク先は日本語版（`/ja/`）を優先。** 日本語版が無い・品質が低いページは英語版でよい。MDNの `/ja/` は未翻訳ページだと英語版へリダイレクトされるため、挿入後にURLの実在とリダイレクト有無を確認する
- **URLが安定しているページに張る。** MDNは安定しているが、フレームワーク公式（Next.js等）はドキュメント構成の変更が多いため、深い階層よりトップに近いページを選ぶ
- **存在しないURLを張らない。** AIがリンクを挿入する場合、もっともらしい架空のMDN URLを生成する事故が典型的に起きる。挿入した外部URLは全件、実際にアクセスして確認する
- 変わりやすい情報（料金・ランタイム・対応状況・制限事項）は従来どおり「公式で確認する」導線を必ず残す（前節を参照）

MDX内では通常のMarkdownリンクで書けばよい。外部リンクの `target="_blank"` や外部リンクアイコンの付与は、`src/components/mdx-content.tsx` の `a` 要素上書きで一律に処理する方針とし、MDX側では特別な記法を使わない。

### 章の目次（TOC）は自動生成される

各章の `h2` / `h3` 見出しは、Velite ビルド時（`velite.config.ts` の `extractToc`）に自動抽出され、章ページに目次として表示される。id は `rehype-slug` と同じ `github-slugger` で生成しているため、本文の見出し id と常に一致する。

- **デスクトップ（`xl` 以上）** — 本文右に追従表示（スクロールに応じた現在地ハイライト付き）
- **モバイル〜`lg`** — 章タイトル下の折りたたみボックス
- 見出しが3個未満の章では表示されない
- 手書きの「## 目次」セクションは自動目次から除外される

このため、**新規章では手書きの「## 目次」セクションは原則不要**。既存章に残っている手書き目次はそのままでよい（削除は別途の整理タスクで行う）。

他の章から特定セクションへリンクする場合など、見出しテキストの変更でリンクが壊れるのを防ぎたい箇所では、従来どおり見出し直前に明示的なアンカー（`<a id="..."></a>`）を置いてよい。自動目次は見出し自体の id にリンクするため、アンカーは章間リンクの安定化という補助用途に限られる。

#### ページ内リンクのフラグメントに日本語を使わない

**同じページの節へリンクするとき、`](#日本語の見出しid)` と書くとハイドレーションエラーになる。** 見出しの id は日本語のまま生成されるため、自動目次の内部リンクをそのまま真似して本文に書いてしまいやすいが、本文のMarkdownリンクで同じことをすると、サーバーが返したHTMLとクライアントの描画が食い違い、コンソールに `Hydration failed because the server rendered HTML didn't match the client` が出る。ページは表示されるので気づきにくい。

```mdx
誤: 配り方は[テーマを配る](#テーマを配る--themeproviderと入れ子)で扱います。
正: <a id="theme-provider"></a> を見出し直前に置き、[テーマを配る](#theme-provider) とリンクする
```

**ページ内リンクを書くときは、見出し直前にASCIIの明示アンカーを置き、そのidを指す。** 自動目次（右側や折りたたみのTOC）は内部実装なので日本語idのままで問題ない。手書きするリンクだけがこの制約を受ける。

検出方法は、章を開いてブラウザのコンソールを見るのが確実。MDX側からは次で探せる。

```bash
rg -n '\]\(#[^)]*[^\x00-\x7F]' content/books/
```

`rg`（ripgrep）が無い環境では GNU grep でも同じことができる。

```bash
grep -rnP '\]\(#[^)]*[^\x00-\x7F]' content/books/
```

**macOS標準の `/usr/bin/grep` は `-P` を受け付けない**（`invalid option -- P` になる）ので、そのときは `rg` を使う。どちらも該当が無ければ終了コード1で何も出力しない。
...
新しい本では、サイドバーの本内検索で使う候補も `src/app/books/_constants/searchSuggestions.ts` に追加する。候補は章タイトルの言い換えではなく、読者が実際に検索しそうな主要キーワードに絞る。追加後は対象の本ページを開き、検索欄をフォーカスしたときに候補が表示されることを確認する。
...
教科書コンテンツでは、重要な定義・判断基準・ハマりやすい注意点などに、必要に応じて `<Marker>` を入れる。
...
基本は1文または文中の短いフレーズに使う。長い文章に付けたい場合は複数行で書いてもよいが、段落全体・箇条書き全体・表・コードブロックを丸ごと囲まない。
...
使いすぎるとページ全体が騒がしくなり、どこが本当に重要なのか分かりにくくなる。
章の内容を見て、読者の理解に役立つ箇所へ絞って使う。
...
> **改善タスクの指示書集は [docs/content/ai-tasks.md](./content/ai-tasks.md) にある。** SEO・UX改善（draft運用、JSON-LD、QuizLink一括挿入、公式リンク横展開、sitemap改善、OG画像、GSC分析）は優先度順の指示書をコピペして委任する。委任時は「devサーバーは1セッションだけ」「同じ本を並行編集しない」を必ず守る。

### 規約の優先度 — チェックリストではなくガードレール

本ファイルの執筆ルールは、すべてが同じ重みではない。**3層に分けて適用する**こと。

1. **必須層（違反は差し戻し）** — 守らないと実害が出るルール。
   frontmatterの形式 / H1なし・H2始まり / draft運用（執筆完了時に `draft: true` を削除） /
   構文・引数表・戻り値の明示 / 内部・外部リンクのルール / 外部URLの実在確認 / 章末のクイズ導線
2. **原則層（外すなら理由が要る）** — 基本は守るが、題材に合わなければ省略してよい。
   ビジュアル要素の密度（3〜4画面に1つ） / 「よくあるハマりどころ」の節 / 次章への橋渡し文
3. **裁量層（題材次第・数合わせ禁止）** — 「効く場所に入れる」もの。規定数を満たすこと自体に価値はない。
   吹き出しの数と位置 / Mermaid図の有無 / `<Marker>` / `<Callout>` の使用量

**最悪の失敗は「形式チェックは通るが中身が浅い章」**である。構成ルールはSEOの直接要因ではなく、
検索意図に答える中身を支える間接装置にすぎない。吹き出しやイラストを「規定数を満たすため」に
入れるくらいなら省くこと。読者がテンプレ臭を感じた瞬間に、これらの装置の効果は消える。
...
- 重要な定義・判断基準・ハマりやすい注意点には、必要に応じて <Marker> を入れる
...
- **`<Marker>` は重要文の視線止めとして使う。** 定義・クイズで問われそうな要点・実務上の判断基準・ハマりやすい注意点に入れる。段落全体を囲むのではなく、読者に覚えてほしい1文または短いフレーズに絞る
...
> `<Marker>` は、読者が流し読みしても重要な箇所を拾いやすくするための強調。定義・ハマりやすい注意点・実務判断の軸などに使う。`<Callout>` のような補足枠ではないので、長い段落を囲まず「覚えてほしい1文」に絞る。
...
ただし、表は「比較・一覧・引数の整理」に使う。長い説明文や注意点をセルへ詰め込み、横スクロールしないと読めない状態にしない。説明列が1〜2文を超える場合は、表では短い要約だけを書き、詳しい説明は表の下の本文・箇条書き・`<Callout>` に逃がす。メソッド名を多数並べるセルでは、意味のまとまりごとに行を分けるか、章の節へ分割する。
...
新しい本を追加したら、以下の6つをすべて更新する。コンテンツファイル（`content/books/`）の追加だけではVeliteが自動検出するが、テーマ・表示順・告知はコード側の設定が必要。**1つでも飛ばすと、本は公開されているのに一覧で色が付かない・お知らせに出ないという中途半端な状態になる。最後の項目まで実行すること。**

### 1. テーマカラーの設定（`src/lib/book-theme.ts`）
...
新しい本のスラッグをキーにして、サイドバー検索の候補語を追加する。候補は多すぎると目次を圧迫するため、主要概念・頻出語・クイズで探されやすい語を10〜15件程度に絞る。

**この項目だけは、新しい本を追加したとき以外にも随時更新してよい。** 既存の本に章を追加したときや、あとから「この語で探されそうだ」と気づいたときは、そのつどサジェストを足す。判断基準と入れ替えの考え方は「[章を追加したら検索サジェストを見直す](#章を追加したら検索サジェストを見直す)」を参照。

```ts
export const BOOK_SEARCH_SUGGESTIONS: Record<string, readonly string[]> = {
  'new-book-slug': ['主要語1', '主要語2', 'よく検索される語'],
};
```

追加後は、対象の `/books/{bookSlug}` または章ページを開き、検索欄をフォーカスしたときに候補が表示されることを確認する。

### 6. トップページのお知らせ（`src/app/page.tsx`）
