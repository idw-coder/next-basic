...
<SpeechBubble character="/images/fork_suit_man_color.png" name="先生" side="right">
  Security
  Groupは「通信を通すか」、WAFは「HTTPリクエストの中身を許可するか」、Shieldは「大量攻撃に耐えるか」を担当します。
</SpeechBubble>
...
DBパスワードやAPIキーをEC2の環境変数、AMI、コンテナイメージ、ソースコード、GitHubリポジトリに直接置くのは避けます。AWSでは**Secrets Manager**と**Systems Manager Parameter Store**の2つで管理するのが基本です。

### Secrets Manager

認証情報のライフサイクル全体（保存・取得・ローテーション）を管理するサービスです。

典型的な用途は**RDSのマスターパスワード管理**です。
...
本番環境のECSタスクやLambdaでは、環境変数にパスワードを直書きする代わりに、起動時にSecrets ManagerのAPIを呼んで認証情報を取得するコードを書くのが一般的です。
AWS SDKを使えば数行で実装でき、取得した値をそのままDBクライアントの接続設定に渡します。
GitHubにパスワードが混入するリスクがなくなり、ローテーションも自動化できるため、セキュリティ要件が厳しい現場ではほぼ必須の構成です。

</Callout>

RDS・Redshift・DocumentDBなどのマネージドDBとネイティブに連携し、**自動ローテーション**を設定できます。ローテーション時はLambda関数が裏で動いてDBのパスワードを変更・同期するため、アプリ側の設定変更なしに常に最新の認証情報を使い続けられます。
...
機密性が必要な値は**SecureString**タイプとしてKMSで暗号化して保存できます。
