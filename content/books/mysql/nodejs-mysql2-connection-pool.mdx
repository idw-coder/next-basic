...
<Callout type="note" title="mysql2/promise を使う">
  `mysql2` にはコールバック版とPromise版があります。現在のNode.jsで書くなら、`mysql2/promise`
  をインポートして `await` で書く形が素直です。この章のコードはすべてPromise版です。
</Callout>
...
<Marker>処理が終わった接続は切断せず、再利用できる状態に戻す。</Marker>
...
<Callout type="info" title="createPool は接続しない">
  `createPool`
  を呼んだ瞬間に接続が10本張られるわけではありません。接続は**必要になったときに作られ**、`connectionLimit`
  に達するまで増えていきます。だから `createPool` に `await`
  は要りませんし、MySQLが起動していなくてもこの行自体はエラーになりません。接続エラーは最初のクエリで初めて表面化します。
</Callout>
...
| `maxIdle`            | `connectionLimit` と同値 | アイドル接続を何本まで保持するか <br/> ※ アイドル接続は、「DBとの接続自体は維持されているが、現在はSQL実行に使われていない接続」 |
...
<SpeechBubble character="/images/oksign_man_color.png" name="先生" side="right">
  `waitForConnections` は `true` のままでいい。`false`
  にすると、混雑した瞬間にリクエストがエラーになる。待たせたほうがマシな場面がほとんどだよ。
</SpeechBubble>
...
<Callout type="warning" title="値を文字列連結しない">
  ...
  のように書くと、SQLインジェクションの入口になります。必ず `?`
  を使い、値は第2引数で渡してください。仕組みそのものについては
  [SQLインジェクション対策](/books/sql-basics/indexes-transactions-and-security)
  を参照してください。
</Callout>
...
<Callout type="note" title="pool.query は借りて返すまでを1行でやっている">
  `pool.query()` は内部で「接続を借りる → クエリを投げる →
  接続を返す」を自動でやってくれます。だから返却を書かなくても接続は漏れません。**ただし、これが次の落とし穴の原因になります。**
</Callout>
...
<Callout type="warning" title="release を忘れると静かに死ぬ">
  `release()` を書き忘れた接続は、プールに戻らないまま占有され続けます。`connectionLimit`
  が10なら、10回そのコードを通った時点でプールが枯渇し、以降のリクエストは全部待ち状態になります。エラーは出ず、ただ応答が返らなくなる——原因を追いにくい壊れ方なので、`finally`
  に書く習慣を最初から付けてください。
</Callout>
...
<Callout type="info" title="まずは既定の10のままでよい">
  `connectionLimit`
  の既定値10は、多くのWebアプリにとって妥当な出発点です。MySQLは接続1本につきスレッド1本を使うので、大きくしすぎるとサーバー側の負荷が上がり、かえって全体が遅くなります。増やすのは「プール待ちが実際に発生している」ことを確認してからにしてください。
</Callout>
...
<Callout type="note" title="Webサーバーでは end を呼ばない">
  `pool.end()`
  はプールを閉じるメソッドです。使い終わりが明確なバッチ処理では最後に呼びますが、常駐するWebサーバーでは呼びません。呼ぶとしても、シャットダウン処理の中だけです。
</Callout>
