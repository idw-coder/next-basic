...
[前の章](/books/mysql/connect-and-max-connections)では、`SHOW PROCESSLIST` に `Sleep` の接続が何本も並んでいる様子を見ました。
...
[第1章](/books/mysql/what-is-mysql)で見た通り、接続の確立にはTCPのハンドシェイクと認証の往復があります。
...
「大きいほど速い」ではありません。ここは前章の `max_connections` と直結します。
...
前章で見た `wait_timeout`（既定8時間）を思い出してください。**サーバーは、長時間使われない接続を一方的に切ります。**
