...
<Marker>`COALESCE` は、表示や計算で扱いやすい代替値を作るための関数です。元データの `NULL` を更新するものではありません。</Marker>
...
<Figure src="/images/deta_man_color.png" caption="WHERE・ORDER BY・LIMITを組み合わせることで、必要なデータだけを効率よく絞り込めます" />
...
「最新10件」「金額が高い順に10件」のように意味のある件数制限にするには、必ず `ORDER BY` で並び順を指定します。
</Callout>
