...
ここまでの章で、Gitの考え方とコマンドを一通り学びました。この章は、それらを「やりたいこと」から**逆引きできる**チートシートです。コマンドを忘れたとき・手早く確認したいときのリファレンスとして使ってください。各セクションの末尾に、詳しい解説のある章へのリンクを置いています。
...
表のなかで<Pick />が付いている行は、日常の作業でとくに出番が多いものです。迷ったらここから覚えてください。
...
| <Pick />作って切り替えを同時に | `git switch -c ブランチ名` |
| <Pick />直前にいたブランチに戻る | `git switch -` |
...
| ブランチを統合する | `git merge 取り込みたいブランチ名` |
| ブランチの根元を付け替える | `git rebase ブランチ名` |

`git merge` では、**現在いるブランチが統合先**です。コマンドに指定したブランチの変更を、現在のブランチへ取り込みます。
...
この例では、`feature/login` の変更を `main` に取り込みます。
...
| <Pick />ステージから下ろす（変更は残す） | `git restore --staged ファイル名` |
| 特定コミットの内容でファイルを復元 | `git restore --source=コミットID ファイル名` |
| <Pick />直前のコミットを取り消す（変更は残す） | `git reset --soft HEAD^` |
...
| <Pick />リモートで消えたブランチを掃除する | `git fetch --prune` |
| リモートの最新を取得してマージ | `git pull` |
| <Pick />リベース型で取り込む | `git pull --rebase` |
| 自分の変更を送る | `git push` |
...
| ------------ | -------- |
| 変更を退避する | `git stash` |
| <Pick />メッセージを付けて退避する | `git stash push -m "メッセージ"` |
...
| <Pick />退避から特定のファイルだけ復元する | `git restore --source=stash@{0} -- src/...` |
...
| <Pick />HEADが動いた全履歴を見る | `git reflog` |
...
| pushしたコミットを取り消したい | `git reset --soft HEAD^` → 再commit → `git push --force-with-lease`（[第8章](/books/git-basic/08-remote-sync)） |
...
- <Marker>このページは「やりたいこと」からコマンドを引く逆引きリファレンス</Marker>
...
Gitを使いこなせるようになったら、次はpush・Pull Requestをきっかけにテストやデプロイを自動化する[『GitHub ActionsでCI/CDをはじめる』](/books/github-actions/01-what-is-cicd)がおすすめです。
