...
間違えてコミットしちゃった…！`reset` と `revert`、どっちで戻すのが正解なの？
</SpeechBubble>
...
<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
`restore` で消した変更は本当に戻せないの？ `reflog` でも無理？
</SpeechBubble>

<SpeechBubble character="/images/oksign_man_color.png" name="先生" side="right">
コミットしていない変更は `reflog` にも残りません。`restore` は「まだGitに記録されていない編集を捨てる」ので、本当に消えます。大事な変更は先にコミットしてから整理する癖をつけましょう。
</SpeechBubble>
...
<Figure src="/images/toppa_suit_woman_color.png" alt="困難を突破する人" width={180}>
取り消し操作は怖く感じますが、仕組みを理解すれば安全に使えます。reflog という最終防衛ラインもあるので安心してください。
</Figure>
...
<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
コミットメッセージにタイポがあった…！コミットし直すしかないの？
</SpeechBubble>

<SpeechBubble character="/images/relief_man_color.png" name="先生" side="right">
`git commit --amend` で直前のコミットメッセージを書き換えられます。ただし push 済みの場合は履歴書き換えになるので、ローカルだけのコミットに使うのが安全です。
</SpeechBubble>
...
- <Marker>**未コミットの変更を戻すなら `restore`**</Marker>
