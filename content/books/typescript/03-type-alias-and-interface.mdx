...
`type` はプリミティブ、ユニオン型、タプル、関数型など、あらゆる型に名前を付けられます。`interface` はオブジェクトの形状（と[クラス](/books/typescript/classes-and-implements)）にしか使えません。
...
[次の章](/books/typescript/classes-and-implements)では、この `interface` をクラスへの契約として使う **implements** を扱います。
