...
- [ベースとなる型](#base-type)
- [Partial\<T\> — 全プロパティをオプショナルにする](#partial)
- [Required\<T\> — 全プロパティを必須にする](#required)
- [Pick\<T, K\> — 特定のプロパティだけ取り出す](#pick)
- [Omit\<T, K\> — 特定のプロパティを除外する](#omit)
- [Record\<K, V\> — キーと値の型を指定したオブジェクト](#record)
- [Readonly\<T\> — 全プロパティをreadonlyにする](#readonly)
- [ReturnType\<T\> — 関数の戻り値の型を取得する](#returntype)
- [ユーティリティ型の組み合わせ](#combining-utility-types)
- [ユーティリティ型の早見表](#utility-types-cheatsheet)
- [まとめ](#summary)
