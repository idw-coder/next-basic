...
<Figure src="/images/calling_suit_man_color.png" alt="依存配列の落とし穴に気づくイメージ" maxWidth="120px" />
...
<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
対処法を全部試しても依存配列の警告が消えないんだけど、`eslint-disable` で黙らせていい？
</SpeechBubble>

<SpeechBubble character="/images/oksign_man_color.png" name="先生" side="right">
最終手段としてはアリだけど、使うなら「なぜ安全なのか」をコメントに残すこと。初学者が「警告がうるさいから」で消すのは、stale closureバグの温床になるから避けよう。
</SpeechBubble>
...
<QuizLink
  ...
  title="Reactクイズに挑戦する"
  description="exhaustive-deps、stale closure、依存配列の抜け漏れ、useCallbackでの参照安定を4択クイズで確認しよう"
/>
