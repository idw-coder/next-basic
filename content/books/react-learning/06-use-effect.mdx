...
<Figure src="/images/pc-work-woman01.png" alt="副作用の後始末を整理するイメージ" maxWidth="120px" />
...
<Callout type="info" title="ESLintルール react-hooks/set-state-in-effect">
  このアンチパターンは、`eslint-plugin-react-hooks` v6で追加されたルール `react-hooks/set-state-in-effect` でも警告されます。エフェクト内で同期的に `setState` を呼んでいるコードを検出し、「レンダー中に計算するか、イベントハンドラに移せないか」を促すルールです。この警告が出たら、まず上のNG/OK例のように書き換えられないか検討してください。
</Callout>
...
<QuizLink
  ...
  title="Reactクイズに挑戦する"
  description="useEffect、依存配列、クリーンアップ関数、不要なEffect、無限ループの原因を4択クイズで確認しよう"
/>
