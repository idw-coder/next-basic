...
`z.coerce.number()` は変換してから検証するため、空文字も `null` も**エラーにならず `0` になります**。フォームの空欄が0で保存される事故につながるので、変換系は別の注意が必要です。詳しくは [z.coerceで文字列を変換する](/books/zod/coercion-and-env) の章で扱います。
