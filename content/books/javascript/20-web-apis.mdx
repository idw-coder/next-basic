...
description: 'fetch API・localStorage/sessionStorage・IntersectionObserver・MutationObserverを、用途・構文・戻り値・注意点まで整理します。'
order: 20
---

ブラウザには、JavaScript本体とは別に**Web API**が用意されています。この章で扱うのは、HTTP通信を行う `fetch`、ブラウザに小さなデータを保存する `localStorage` / `sessionStorage`、要素が画面に入ったかを検知する `IntersectionObserver`、DOMの変更を監視する `MutationObserver` です。
...
`Array`、`Map`、`JSON`、`Date` はJavaScript言語の標準機能です。一方、`localStorage`、`document.querySelector`、`IntersectionObserver` はブラウザ環境が提供するWeb APIです。Node.jsにはDOMやStorageがありません。Node.js 18以降では `fetch` を使えますが、CookieやCORSの扱いはブラウザと一致しません。
</Callout>
...
<Figure src="/images/pc-work-woman01.png" alt="ブラウザ上でWeb APIを使っているイメージ" maxWidth="180px" />
...
`response.ok` はステータスコードが `200` 〜 `299` のとき `true` です。API通信では `await fetch(...)` の直後に `response.ok` を確認し、失敗時の処理を明示します。詳しい例外処理は[エラーハンドリング](/books/javascript/14-error-handling)で扱っています。
</Callout>
...
`response.json()` や `response.text()` は本文ストリームを消費します。同じ `Response` に対して複数回読むと失敗します。ログ用と処理用で両方必要な場合は、先に `const text = await response.text()` で文字列として読み、必要に応じて `JSON.parse(text)` します。
</Callout>
...
`localStorage` と `sessionStorage` はブラウザのJavaScriptから読めます。アクセストークン、パスワード、個人情報、決済情報のような秘密情報は保存しません。認証情報を扱う場合は、JavaScriptから読めない `HttpOnly` Cookieを使います。
</Callout>
...
<Figure src="/images/deskwork_check_man_color.png" alt="表示領域に入った要素を確認するイメージ" maxWidth="140px" />
...
自分のアプリが変更している状態は、変数、React state、URL、サーバーデータで管理します。`MutationObserver` は、外部スクリプト、ブラウザ拡張、埋め込みウィジェットがDOMを書き換える場面で使います。
</Callout>
