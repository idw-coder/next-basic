...
`new Date(2026, 0, 1)` は「2026年**1月**1日」。月を指定するときは**実際の月から1を引く**、取得するときは**1を足す**（後述の `getMonth()`）と覚えておくと事故りません。
</Callout>
...
`setDate` などのセッターは**元の `Date` オブジェクトを直接変更**します。元の日付を残したいときは `new Date(original)` でコピーしてから操作してください。
</Callout>
...
`Date` は月0始まり・破壊的なセッター・タイムゾーンの扱いにくさなど、設計上の弱点が指摘されてきました。これを解決する新しい標準API **Temporal** が策定中で、イミュータブルで直感的な日時操作を提供します。現時点で実務に使うなら、軽量な **Day.js** や関数型の **date-fns** といったライブラリが定番です。日付処理が複雑になってきたら、自前の `Date` 計算に固執せずライブラリ導入を検討しましょう。
</Callout>
...
<Figure src="/images/deskwork_check_man_color.png" alt="日付処理のハマりどころを確認するイメージ" maxWidth="120px" />
