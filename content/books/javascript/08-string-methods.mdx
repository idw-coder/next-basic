...
<Figure src="/images/deta_man_color.png" alt="文字列を加工して新しい文字列を作るイメージ" maxWidth="120px" />
...
各メソッドは、まず `メソッド名(引数1, 引数2?)` という形（**シグネチャ**）を示し、続けて**引数の表**と「戻り値」を載せます（`引数?` のように `?` が付くものは**省略できる引数**）。引数は一番知りたい情報なので、地の文に埋もれさせず**表でパッと確認**できるようにしています。MDNのリファレンス代わりに使ってください。
</Callout>
...
`slice` はマイナス（末尾からの指定）が使えますが、`substring` は使えません（マイナスは0扱い）。迷ったら **`slice` に統一**するのがおすすめです。
</Callout>
...
<Callout type="info">
「含まれるか」を知りたいだけなら、`indexOf('x') !== -1` より **`includes('x')`** の方が意図が明確です。
</Callout>
...
| 引数 | 渡せるもの | 説明 |
| ---- | ---------- | ---- |
| `pattern`（第1引数） | 文字列 / 正規表現 | **何を**置き換えるか。**文字列を渡すと最初にマッチした1件だけ**が対象になる（要注意） |
...
フォームのメールアドレスや検索キーワードは、`value.trim().toLowerCase()` で前後空白の除去＋小文字化をしてから比較・保存するのが定番です。「末尾にスペースが入っていてログインできない」事故を防げます。
</Callout>
...
<Figure src="/images/deskwork_check_man_color.png" alt="文字列操作のハマりどころを確認するイメージ" maxWidth="120px" />
