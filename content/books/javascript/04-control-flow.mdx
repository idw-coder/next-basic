...
<Marker>`entries()` が「インデックスと値のペア」を1つの配列として返しているからです。</Marker>
...
受け取るのが配列なら、`entry[0]` と `entry[1]` でも取り出せます。それを1行で書けるようにしたのが**分割代入**です。次の2つはまったく同じ動きをします。
...
左辺の `[index, item]` は配列を作っているのではなく、**右辺の配列を先頭から順に取り出して名前を付ける**記法です。1周目には `[0, 'a']` が渡ってくるので、`index` に `0`、`item` に `'a'` が入ります。

<Figure
  src="/images/fork_suit_man_color.png"
  alt="1つの配列を2つの変数に振り分けるイメージ"
  maxWidth="120px"
/>

つまり `for (const [index, item] of array.entries())` が動くのは、`entries()` がペアの配列を返し、それを分割代入が2つの変数へ振り分けているためです。

<Callout type="info" title="同じ形はこの先も何度も出てくる">
`[キー, 値]` のペアを分割代入で受け取る書き方は、`Object.entries()` や [`Map`](/books/javascript/17-map-set-weakref) の反復でもそのまま使います。ここで形を押さえておくと、後の章が読みやすくなります。
</Callout>
...
<Callout type="warning" title="配列に for...in は使わない">
  `for...in` はプロトタイプチェーン上のプロパティも列挙する可能性があります。配列の反復には `for...of` または [`forEach`](/books/javascript/07-arrays1-transform-methods#foreach) を使いましょう。
</Callout>
...
| 配列の各要素に副作用を実行する | [`forEach`](/books/javascript/07-arrays1-transform-methods#foreach) |
...
- [MDN — Array.prototype.entries()](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/entries)
