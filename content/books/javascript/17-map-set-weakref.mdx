...
一見すると[オブジェクト（`{}`）](/books/javascript/06-objects)と同じに思えますが、決定的な違いがあります。

オブジェクトのキーは文字列（またはSymbol）に限定されるのに対し、Mapは **オブジェクト・関数・数値など、あらゆる型をキーにできます**。

他に個人的にポイントとなる特徴は、[便利なメソッド（`get`、`set`、`has`、`delete`、`clear`、`forEach`、`keys`、`values`、`entries`）](/books/javascript/17-map-set-weakref#mapの主要メソッド)があることです。
...
| `keys()`          | キーのイテレータを取得         | イテレータ          |
| `values()`        | 値のイテレータを取得           | イテレータ          |
| `entries()`       | ペアのイテレータを取得         | イテレータ          |

<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
  `forEach` って `(値, キー)` の順番なんですか？直感と逆な感じがして…
</SpeechBubble>
...
Mapは `for...of` で回せます（このような性質を「イテラブル」と呼びます。詳しい仕組みは[次章のイテレータとジェネレータ](/books/javascript/18-iterators-and-generators)で解説します）。各要素は `[key, value]` の配列として取り出せます。
挿入順が保持されるのはSetと同じです。
...
<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
  Map って `.map()` や `.filter()` は使えないんですか？
</SpeechBubble>

Mapは配列ではないので、そのままでは使えません。ただし、スプレッド構文や `Array.from` で配列に変換すれば使えます。

<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
  あ、そっか…Map って配列じゃないんですね。ずっと配列みたいなものだと思ってました！
</SpeechBubble>
...
console.log(labels); // ["a=1", "b=2", "c=3"]
```
...
SetもMapと同じく[イテラブル](/books/javascript/18-iterators-and-generators)なので、`for...of` やスプレッド構文で回せます。挿入順が保持されます。
