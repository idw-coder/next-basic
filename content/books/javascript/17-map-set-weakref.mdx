...
<Figure src="/images/deskwork_check_man_color.png" alt="キーの同値判定を確認するイメージ" maxWidth="120px" />
...
`get` で存在しないキーを参照すると `undefined` が返ります。値として `undefined` を格納している場合と区別がつかないため、キーの存在確認には必ず `has` を使いましょう。
</Callout>
...
<Figure src="/images/fork_suit_man_color.png" alt="ObjectとMapの使い分けを考えるイメージ" maxWidth="120px" />
...
迷ったら「キーが文字列で、構造が固定なら Object」「キーが動的・非文字列、頻繁に追加削除するなら Map」と覚えると実務で困らないよ。
</SpeechBubble>
...
<Callout type="note">
`counter.get(word) ?? 0` は、キーが未登録（`undefined`）なら `0` を使う、というイディオムです。Objectで同じことをすると `obj[word] = (obj[word] || 0) + 1` と書きますが、`||` は `0` も偽値として扱ってしまうため `??`（Nullish Coalescing）の方が安全です。
</Callout>
...
<SpeechBubble character="/images/oksign_man_color.png" name="先生" side="right">
手動で `delete` するのは漏れやすい。WeakMap なら「不要になったら勝手に消える」から、メモリリークのリスクがそもそもないんだ。
</SpeechBubble>
...
WeakRef と FinalizationRegistry は日常的に使う機能ではありません。ライブラリやフレームワークの内部実装で使われることがほとんどで、アプリケーションコードで直接使う場面はまれです。ここでは「こういうものがある」という理解にとどめれば十分です。
</Callout>
