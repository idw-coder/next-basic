...
配列メソッドの多くは**コールバック関数**を引数に取ります。そのコールバックは `(element, index, array)` の3つの引数を受け取れる、という形が共通です（`index`・`array` は省略可能で、使わなければ書かなくてかまいません）。各メソッドでは「コールバックが何を返すと、メソッド自身が最終的に何を返すか（戻り値）」に注目すると整理しやすくなります。
</Callout>
...
<Figure src="/images/boomerang_woman_color.png" alt="mapが変換結果を新しい配列として返すイメージ" maxWidth="120px" />
...
<Figure src="/images/deskwork_check_man_color.png" alt="配列メソッドのハマりどころを確認するイメージ" maxWidth="120px" />
