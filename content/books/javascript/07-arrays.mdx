...
description: 'map・filter・reduce・find・some・every・slice・concatなど、配列メソッドを用途別に整理し、実務でよく書くパターンをまとめます。破壊的メソッド・非破壊メソッドの一覧表つき。'
order: 7
---
...
- `map` は変換、`filter` は絞り込み、`reduce` は集約、`slice` は部分取得、`concat` は結合——目的に応じて使い分ける
