...
description: 'map・filter・reduce・find・some・every・slice・concatなど、配列メソッドを用途別に整理し、実務でよく書くパターンをまとめます。'
order: 7
---
...
この章で扱った `map`・`filter`・`reduce`・`find`・`some`・`every`・`slice`・`concat`・`flatMap` はすべて**非破壊**（元の配列を変更しない）です。
