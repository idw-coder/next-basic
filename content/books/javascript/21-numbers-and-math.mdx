...
メソッドは `メソッド名(引数1, 引数2?)` という形（シグネチャ）を示し、続けて**引数の表**と「戻り値」を載せます（`引数?` は省略できる引数）。MDNのリファレンス代わりに、「何を渡すと何が返るか」を確認しながら使ってください。
</Callout>

## 目次

- [JavaScriptの数値は基本「1種類」だけ — number 型](#section-1)
- [文字列と数値の相互変換](#section-2)
- [浮動小数点の罠 — 0.1 + 0.2 が 0.3 にならない](#section-3)
- [数値のフォーマット — toFixed / toLocaleString](#section-4)
- [Number の静的メソッド・プロパティ](#section-5)
- [Math オブジェクト — よく使う計算](#section-6)
- [早見表](#section-7)
...
厳密には `number` のほかに `BigInt`（`10n` のように末尾に `n` を付ける）という整数専用の型もあります。`number` が安全に扱える整数の上限（`Number.MAX_SAFE_INTEGER` ≒ 9000兆）を超える巨大な整数を扱うときに使いますが、日常的な計算ではまず `number` だけで十分です。
</Callout>
...
`parseInt('08')` のように基数を省略すると、環境によって意図しない解釈をされる歴史的な落とし穴があります。**`parseInt(str, 10)` のように常に `10` を明示**するのが鉄則です（10進数として読む、の意味）。
</Callout>
...
「1234567 → 1,234,567」のようなカンマ区切りは、正規表現で自作せず `toLocaleString()` に任せるのが定番です。通貨記号やパーセント表示も `options` で対応できます。
</Callout>
...
古い `isNaN('abc')` は、引数をいったん数値に変換してから判定するため `true` を返してしまい直感に反します。`Number.isNaN()` は変換せず厳密に判定するので、**`Number.isNaN()` を使う**のが安全です。
</Callout>
...
<Figure src="/images/planning_suit_man_color.png" alt="数値を計算・加工するイメージ" maxWidth="120px" />
...
<SpeechBubble character="/images/oksign_man_color.png" name="先生" side="right">
`Math.max(...arr)` のスプレッド渡しは頻出パターン。[分割代入とスプレッド構文](/books/javascript/09-destructuring-and-spread)で扱った `...` がここでも活きてくるよ。
</SpeechBubble>
...
<Figure src="/images/deskwork_check_man_color.png" alt="数値処理のハマりどころを確認するイメージ" maxWidth="120px" />
