...
<Callout type="warning" title="Webフォントは読み込みコストも見る">
  太さを何種類も読み込むと、初回表示が遅くなることがあります。本文・中見出し・強調に使う `400`,
  `500`, `700` 程度から始め、必要以上に増やさないようにしましょう。
</Callout>
...
<Callout type="column">
  ブラウザのデフォルト `font-size` は通常 16px です。`1rem = 16px`
  として計算できますが、ユーザーがブラウザ設定でフォントサイズを大きくしている場合、`rem`
  はそれに追従します。本文を `px` で固定しすぎると、アクセシビリティを損ないます。
</Callout>
...
<Callout type="info" title="文字サイズだけで解決しない">
  「読みづらい」と感じる原因は、文字サイズではなく行長や行間にあることも多いです。まず
  `font-size`、`line-height`、`max-width` の3つをセットで確認しましょう。
</Callout>
...
<Callout type="warning" title="break-allの使いすぎに注意">
  `word-break: break-all;` は便利ですが、文章の読みやすさを大きく下げることがあります。まずは
  `overflow-wrap: break-word;` を試し、それでも狭い領域で崩れる場合だけ局所的に使いましょう。
</Callout>
...
- <Marker>本文の `font-size` は `1rem` を基準にし、ユーザーのブラウザ設定を尊重する</Marker>
...
- [MDN — font-family](https://developer.mozilla.org/ja/docs/Web/CSS/Reference/Properties/font-family) — フォント指定と総称ファミリーのリファレンス
- [MDN — line-height](https://developer.mozilla.org/ja/docs/Web/CSS/Reference/Properties/line-height) — 行間指定のリファレンス
- [MDN — white-space](https://developer.mozilla.org/ja/docs/Web/CSS/Reference/Properties/white-space) — 改行・空白の扱いの全値
- [MDN — overflow-wrap](https://developer.mozilla.org/ja/docs/Web/CSS/Reference/Properties/overflow-wrap) — 長い単語・URLの折り返し制御
