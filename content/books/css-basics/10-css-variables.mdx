...
- [基本の書き方 — 定義と参照](#section-1)
- [デフォルト値（フォールバック）](#section-2)
- [スコープ — 変数が有効な範囲](#section-3)
- [デザイントークンとしてのCSS変数](#section-4)
- [ダークモード対応](#section-5)
- [コンポーネント設計でのCSS変数](#section-6)
- [JavaScriptとの連携](#section-7)
...
CSS変数の命名は `--カテゴリ-用途` の形が見通しやすいです。`--color-primary`、`--spacing-md`、`--radius-lg` のように。「この値を変えると何に影響するか」が名前から分かるのが理想です。
</Callout>
...
Tailwind CSSやshadcn/uiなどのモダンなフレームワークも、内部的にCSS変数を活用してテーマカラーを管理しています。CSS変数の仕組みを理解しておくと、フレームワークのカスタマイズも容易になります。
</Callout>
...
<Figure src="/images/deskwork_check_man_color.png" alt="コンポーネント設計を確認するイメージ" maxWidth="180px" />
...
:root { --spacing: 16; }
.box { padding: calc(var(--spacing) * 1px); } /* 動くがやめた方がいい */
