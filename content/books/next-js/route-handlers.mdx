...
<Figure src="/images/pc-work-woman01.png" alt="Next.jsでAPIエンドポイントを実装しているイメージ" maxWidth="260px" />
...
同じURLセグメントに `page.tsx` と `route.ts` を同時に置くことはできません。`/users` は画面、`/api/users` はAPI、のように役割ごとにURLを分けましょう。
</Callout>
...
Server Componentから同じアプリ内のAPIを `fetch` するより、DBアクセスやサービス関数を直接呼ぶ方がシンプルなことも多いです。Route Handlerは「ブラウザ、外部サービス、モバイルアプリなど、HTTP経由で呼ばれる入口」が必要なときに特に向いています。
</Callout>
...
<SpeechBubble character="/images/usingcomputer_suit_woman_color.png" name="学習者">
同じURLなのに、`GET` と `POST` で別の処理になるんだね。URLだけじゃなくて、HTTPメソッドもルーティングの条件になるのか。
</SpeechBubble>
...
`GET` でデータを作成・更新・削除するのは避けましょう。ブラウザやクローラ、プリフェッチなどが意図せずGETを実行することがあります。副作用のある処理は `POST` / `PUT` / `PATCH` / `DELETE` に分けるのが基本です。
</Callout>
...
厳密には、`PUT` はリソース全体の置き換え、`PATCH` は一部更新に使われます。ただし小規模なアプリでは、更新処理を `PUT` に寄せても実用上は困らないことが多いです。API設計を丁寧に揃えたい場合は、名前だけ変更するような処理を `PATCH` に分けましょう。
</Callout>
...
<Figure src="/images/deskwork_check_man_color.png" alt="APIレスポンスの内容を確認しているイメージ" maxWidth="220px" />
