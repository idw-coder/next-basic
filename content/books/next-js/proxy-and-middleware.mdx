...
- [proxy.js - File-system conventions | Next.js](https://nextjs.org/docs/app/api-reference/file-conventions/proxy) — `proxy.ts` の置き場所、`matcher`、設定の公式リファレンス（英語）。`middleware` のURLはこのページへリダイレクトされる
- [Codemods | Next.js](https://nextjs.org/docs/app/guides/upgrading/codemods) — `middleware-to-proxy` を含む、バージョン移行用codemodの一覧（英語）
- [Edge Runtime | Next.js](https://nextjs.org/docs/app/api-reference/edge) — Edge Runtimeで使えるWeb標準APIの一覧（英語）
- [NextResponse | Next.js](https://nextjs.org/docs/app/api-reference/functions/next-response) — `redirect` / `rewrite` / `next` とヘッダー・Cookie操作の公式リファレンス（英語）
