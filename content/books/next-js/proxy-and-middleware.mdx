...
<MermaidDiagram id="next-proxy-flow" chart={`
flowchart LR
...
<Marker>Next.js 16では、従来の `middleware.ts` というファイル規約が非推奨になり、`proxy.ts` にリネームされました。</Marker>
...
<MermaidDiagram id="next-proxy-rename-flow" chart={`
flowchart LR
